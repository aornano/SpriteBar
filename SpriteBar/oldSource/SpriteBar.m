...
- (void)resetProgress {
    ...
    self.texture = [self.atlas textureNamed:[NSString stringWithFormat:@"%@_%lu.png",self.textureReference,(long)[self closestAvailableToPercent:0]]];
...
- (void)setProgress:(CGFloat)progress {
    ...
    self.texture = [self.atlas
                    textureNamed:[NSString stringWithFormat:@"%@_%lu.png",self.textureReference,(long)[self closestAvailableToPercent:percent]]];
    
    // If we have reached 100%, invalidate the timer and perform selector on passed in object.
    if (fabs(progress) >= fabs(1.0)) {
